...
def resolve_output_path(symbol: str, output: Optional[str]) -> str:
    """Resolve the JSON output path and ensure the parent directory exists."""
    if output:
        output_path = output
    else:
        safe_sym = symbol.replace(".", "_").replace("/", "_")
        output_path = os.path.join(DEFAULT_OUTPUT_DIR, f"_temp_value_analysis_{safe_sym}.json")

    abs_parent = os.path.dirname(os.path.abspath(output_path))
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="仅指定输出目录；文件名仍按 _temp_value_analysis_<symbol>.json 生成",
    )
    ...
    if args.output and args.output_dir:
        print("❌ --output 与 --output-dir 不能同时使用", file=sys.stderr)
        return 1

    output_override = None
    if args.output_dir:
        safe_sym = args.symbol.replace(".", "_").replace("/", "_")
        output_override = os.path.join(args.output_dir, f"_temp_value_analysis_{safe_sym}.json")
    elif args.output:
        output_override = args.output

    output_path = resolve_output_path(args.symbol, output_override)
